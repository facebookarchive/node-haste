...
var requireDynamicRe = /\brequireDynamic\s*\(\s*[\'"]([^"\']+)["\']\s*\)/g;
function requireDynamicCalls(code) {
  if (code.indexOf('requireDynamic(') === -1) {
    return [];
  }
  return extractStrings(code, requireDynamicRe, 1);
}
...
exports.requireDynamicCalls = requireDynamicCalls;
