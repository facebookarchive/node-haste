...
JSLoader.prototype.extractJavelinSymbols =
  ...
  child.on(
    'exit',
    function() {
      me._javelinSymbolsCallback(js, output, code, messages, callback);
    });
...
JSLoader.prototype._javelinSymbolsCallback =
  function(js, output, code, messages, callback) {
  var defines = {};
  var requires = {};
  ...
  // All Javelin files depend on javelin-magical-init to set up the JX
  // namespace. Javelin just assumes that init is on the page already,
  // so its static analyzer does not require magical-init to be @required
  // everywhere.
