...
/**
 * Syncronous version of resolveRequire
 * Can be used by dependency resolver during execution
 *
 * @param  {String}   required id of required module
 * @param  {String}   current  path to requiring module
 * @return {String}            resolved path
 */
ProjectConfiguration.prototype.resolveRequireSync =
  function(required, current) {
  ...
  if (!path.existsSync(filePath + '.js')) {
    ...
    if (!path.existsSync(filePath + '.js')) {
      ...
      return this.resolveID(filePath) || required;
    }
  } else {
    return this.resolveID(filePath) || required;
  }
