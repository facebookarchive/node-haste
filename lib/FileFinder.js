 ...
 * @class
 * @param  {Object}         options                 Parameter bag of configuration options.
 * @param  {Array.<String>} options.scanDirs        Directories to scan, ex: ['html/']
 * @param  {Array.<String>} options.extensions      Extensions to searc for, ex: ['.js']
 * @param  {function|null}  options.ignore          Optional function to filter out paths
 * @param  {Boolean|null}   options.useNative       Use os find instead of javascript's
 * @param  {Boolean|null}   options.followSymlinks  Whether to follow symlinks.
...
function FileFinder(options) {
  ...
  this.followSymlinks = options && options.followSymlinks || false;
...
/**
 * Find files using the configured finder instance.
 *
 * @param  {Function} callback
 */
FileFinder.prototype.find = function(callback) {
  if (this.useNative) {
    this.findNative(callback);
  } else {
    this.findJS(callback);
  }
};

/**
 * Scans directories for files with given extensions (async)
 * Will optionally follow symlinks. Uses node.js native function to traverse, can
 * be slower, but more safe than findNative
 *
 * @param  {Function}       callback
 */
FileFinder.prototype.findJS = function(callback) {
  var scanDirs = this.options.scanDirs;
  ...
  function resolveLink(statfn, curFile, cb) {
    activeCalls++;
    statfn(curFile, function(err, stat) {
      activeCalls--;
      if (followSymlinks && stat.isSymbolicLink()) {
        resolveLink(fs.stat, curFile, cb);
  ...
  function readdirRecursive(curDir) {
    ...
    fs.readdir(curDir, function(err, names) {
      ...
      names.forEach(function(curFile) {
        ...
        resolveLink(fs.lstat, curFile, function(err, stat) {
          if (!err && stat) {
            if (stat.isDirectory()) {
...
/**
 * Scans directories for files with given extensions (async)
 * Will optionally follow symlinks. Uses native find shell script. Usually faster than
 * node.js based implementation though as any shell command is suspectable to
 * attacks. Use with caution.
 *
 * @param  {Function}       callback
 */
FileFinder.prototype.findNative = function(callback) {
  var scanDirs = this.options.scanDirs;
  var extensions = this.options.extensions;
  var ignore = this.options.ignore;
  var followSymlinks = this.options.followSymlinks;
  ...
  // Follow symlinks. This should fix issues with any npm linked modules,
  // local symlinks (i.e. the ../../../ require problem)
...
FileFinder.find = function(scanDirs, extensions, ignore, callback) {
...
FileFinder.findNative = function(scanDirs, extensions, ignore, callback) {
  return (new FileFinder({
    scanDirs: scanDirs,
    extensions: extensions,
    ignore: ignore,
