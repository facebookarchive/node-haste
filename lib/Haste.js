...
Haste.loadJSModuleDependencies = function(jsModuleIDs, locations, cb) {
  var haste = new Haste(
    [
      // configure all the loaders you need
      new loaders.JSLoader({
        extractSpecialRequires: true // support for requireLazy, requireDynamic
      }),  // JS files
      new loaders.CSSLoader({}), // CSS files
  ...
  // pass in the empty map into update since you don't want a cached version
  haste.updateMap(new ResourceMap(), function(updatedMap) {
    var orderedModuleNames = {};
    for (var i = 0; i < jsModuleIDs.length; i++) {
      var jsModuleID = jsModuleIDs[i];
      var module = updatedMap.getResource('JS', jsModuleID);
      mergeRecursive(updatedMap, module, orderedModuleNames);
    }
    cb(orderedModuleNames);
  });

  function mergeRecursive(map, mod, visited) {
    if (!mod || !mod.id || visited[mod.id]) {
      return;
    }
    for (var i = 0; i < mod.requiredModules.length; i++) {
      var modName = mod.requiredModules[i];
      if (!visited[modName] && modName) {
        mergeRecursive(map, map.getResource('JS', modName), visited);
      }
    }
    visited[mod.id] = map.getResource('JS', mod.id);
  }
